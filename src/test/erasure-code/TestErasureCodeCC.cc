...
TEST(ErasureCodeCC, init_default) //k=2, m=1
{
  std::string directory("directory");
  ErasureCodeCC cc(directory);
...
TEST(ErasureCodeCC, encode_1)
{
  ...
  std::generate_n(&buf[0], _chunk_size, rand);
...
TEST(ErasureCodeCC, decode_1)
{
  ...
  decoded.clear();
  err = cc.decode(set<int>(want_to_decode, want_to_decode + 7), encoded, &decoded);
  EXPECT_EQ(0, err);
  ...
  for(unsigned i = cc.get_data_chunk_count(); i < cc.get_chunk_count(); ++i){
    EXPECT_EQ(_chunk_size, decoded[i].length());
    cmp = memcmp(decoded[i].c_str(), encoded[i].c_str(), _chunk_size);
    EXPECT_EQ(0, cmp);
  }
